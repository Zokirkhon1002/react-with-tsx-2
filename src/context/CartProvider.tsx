...
const useCartContext = (initCartState: CartStateType) => {
  ...
  const totalItems: number = state.cart.reduce(
    (prevValue: number, cartItem: CartItemType) => (prevValue += cartItem.qty),
    0
  );
  ...
  const totalPrice: string = new Intl.NumberFormat("en-US", {
    ...
    state.cart.reduce(
      (prevValue: number, cartItem: CartItemType) =>
        prevValue + cartItem.qty * cartItem.price,
      0
    )
  );

  const cart: CartItemType[] = state.cart.sort(
    (a: CartItemType, b: CartItemType) => {
